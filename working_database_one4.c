...
/*void *display1(void *p)
  {
  pthread_mutex_lock(&lock);                                        //to avoid race around conditions we are using mutex
  printf("enter id1 to display\n");                                 
  scanf("%s",id0);                                                  
//if(id0[0]=='0' && id0[1]=='2' && id0[2]=='1' && id0[3]=='0')
//{
...
{
//printf("hi\n");                                        
if(strcmp(new->EMP_ID,id0)==0)                          //entered id matches with the existing id then prints the user details
{
...
employee_data *temp=head,*t=head;
while((temp->link)!=NULL)
{
if(strcmp(new->Name,temp->Reporting_Manager)==0)
{
printf("%s\t",t->EMP_ID);
printf("%s\n",t->Name);
}
...
while((temp->link)!=NULL)
{
if(strcmp(temp->Reporting_Manager,s))
{
	printf("%s\t",t->EMP_ID);
	printf("%s\n",t->Name);
}
...
void *display3(void *p)
{
	...
	scanf("%s",id2);
	employee_data *new=head;
	if((strcmp(id2,id1)==0)||(strcmp(id2,id0)==0))
	{
		printf("data already enterd\n");
		//pthread_exit(NULL);
	}
